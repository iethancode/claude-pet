...
cd claude-pet
pnpm install
pnpm tauri build      # 产物在 target/release/（workspace 根目录）
```
...
产物在 `target/release/`（项目根是 Cargo workspace，所以 target 在根目录而非 `src-tauri/` 下）：
...
```bash
grep -c "index-.*\.js" target/release/claude-pet.exe   # 正确：≥1；错误：0（target 在项目根）
```
